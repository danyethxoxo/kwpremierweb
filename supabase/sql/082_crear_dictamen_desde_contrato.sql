-- 082_crear_dictamen_desde_contrato.sql
-- Permite buscar contratos finalizados por asesor y usarlos como origen
-- de un dictamen, sin volver a capturar la información compatible.
-- Requiere 078_documentos_dictamen.sql.
...
begin
  ...
  if v_doc.dictamen_id is not null and v_doc.dictamen_id <> p_dictamen_id then
    raise exception 'Este contrato ya está enlazado con otro dictamen.';
  end if;
  if not exists (
